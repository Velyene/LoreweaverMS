...
class CharacterFormMappingsTest {
	...
	fun withAttribute_updatesOnlyRequestedAbility() {
		val updatedState = CharacterFormState().withAttribute("DEX", "14")
	...
	fun quickBuild_withDexPrimary_usesDexArmorPreset() {
		val classInfo = ClassInfo(
			displayName = "Rogue",
			hitDie = 8,
			primaryStats = listOf("DEX", "INT"),
			defaultSaveProficiencies = setOf("DEX", "INT"),
	...
	fun quickBuild_withDexAndStrPrimary_prefersDexArmorPreset() {
		val classInfo = ClassInfo(
			displayName = "Fighter",
			hitDie = 10,
			primaryStats = listOf("DEX", "STR"),
			defaultSaveProficiencies = setOf("STR", "CON"),
			defaultSpellSlotsL1 = emptyMap()
		)
