...
private const val ANIMAL_AC_PREFIX = "$MONSTER_STAT_AC "
private const val ANIMAL_HP_PREFIX = "$MONSTER_STAT_HP "
private const val ANIMAL_SPEED_PREFIX = "$MONSTER_STAT_SPEED "
private const val ANIMAL_CR_PREFIX = "$MONSTER_STAT_CR "
private val animalSectionHeaders = setOf(
	MONSTER_SECTION_TRAITS,
	MONSTER_SECTION_ACTIONS,
	MONSTER_SECTION_BONUS_ACTIONS,
	MONSTER_SECTION_REACTIONS
)
private val animalProfileLabels = listOf(
...
private val animalAbilityRegex =
	Regex("""(?i)\b(Str|Dex|Con|Int|Wis|Cha)\s+(\d+)\s+([+\-−]\d+)\s+([+\-−]\d+)""")
private val animalFooterRegex = Regex("""^\d+\s+System Reference Document 5\.2\.1$""")

object MonsterReferenceDataAnimals {
	val ENTRIES: List<MonsterReferenceEntry> = parseAnimalMonsterEntries()
}

private data class ParsedAnimalEntry(
	val entry: MonsterReferenceEntry,
	val nextIndex: Int
)

private data class ParsedAnimalAbilityFragments(
	val fragments: List<String>,
	val nextIndex: Int
)

private data class ParsedAnimalBody(
	val challengeRating: String,
	val bodyLines: List<String>,
	val nextIndex: Int
)

private fun parseAnimalMonsterEntries(): List<MonsterReferenceEntry> {
	val lines = ANIMAL_MONSTER_RAW.lineSequence()
		.map(String::trim)
		.filter(String::isNotBlank)
		.filterNot { line -> animalFooterRegex.matches(line) }
		.toList()

	val entries = mutableListOf<MonsterReferenceEntry>()
	var index = 0
	while (index < lines.size) {
		if (!isAnimalEntryStart(lines, index)) {
			index++
			continue
		}

		val parsedEntry = parseAnimalEntryAt(lines, index)
		entries += parsedEntry.entry
...
private fun parseAnimalEntryAt(lines: List<String>, startIndex: Int): ParsedAnimalEntry {
	var index = startIndex
	val name = lines[index++]
	val subtitle = lines[index++]
	val (ac, initiative) = parseAcAndInitiative(lines[index++])
	val hp = lines[index++].removePrefix(ANIMAL_HP_PREFIX).trim()
	val speed = lines[index++].removePrefix(ANIMAL_SPEED_PREFIX).trim()

	if (index < lines.size && lines[index].startsWith(ANIMAL_MOD_SAVE_PREFIX)) {
		index++
	}

	val abilityFragments = parseAnimalAbilityFragments(lines, index)
	val parsedBody = parseAnimalBody(
		lines = lines,
		startIndex = abilityFragments.nextIndex,
		abilityLine = parseAnimalAbilityLine(abilityFragments.fragments)
	)

	return ParsedAnimalEntry(
		entry = monster(
			name = name,
			subtitle = subtitle,
			ac = ac,
			...
			hp = hp,
			speed = speed,
			cr = parsedBody.challengeRating,
			body = parsedBody.bodyLines.joinToString("\n")
		),
		nextIndex = parsedBody.nextIndex
	)
}

private fun parseAnimalAbilityFragments(
	lines: List<String>,
	startIndex: Int
): ParsedAnimalAbilityFragments {
	var index = startIndex
	val fragments = mutableListOf<String>()
	while (index < lines.size && !isAnimalBodyControlLine(lines, index)) {
		fragments += lines[index]
		index++
	}
	return ParsedAnimalAbilityFragments(fragments = fragments, nextIndex = index)
}

private fun parseAnimalBody(
	lines: List<String>,
	startIndex: Int,
	abilityLine: String?
): ParsedAnimalBody {
	...
	abilityLine?.let(bodyLines::add)

	while (index < lines.size && !isAnimalEntryStart(lines, index)) {
		val line = lines[index]
		when {
			line.startsWith(ANIMAL_CR_PREFIX) -> {
				challengeRating = line.removePrefix(ANIMAL_CR_PREFIX).normalizeAnimalText()
				index++
			}

			animalProfileLabelFor(line) != null -> {
				val parsedProfileLine = parseAnimalProfileLine(lines, index)
				bodyLines += parsedProfileLine.first
				index = parsedProfileLine.second
			}

			line in animalSectionHeaders -> {
				val parsedSectionLine = parseAnimalSectionLine(lines, index)
				bodyLines += parsedSectionLine.first
				index = parsedSectionLine.second
			}

			else -> {
				bodyLines += line.normalizeAnimalText()
				index++
			}
		}
	}

	return ParsedAnimalBody(
...
private fun parseAnimalProfileLine(lines: List<String>, startIndex: Int): Pair<String, Int> {
	var index = startIndex
	val line = lines[index]
	val label = animalProfileLabelFor(line).orEmpty()
	val value = buildString {
		append(line.removePrefix(label).trim())
		index++
		while (index < lines.size && !isAnimalEntryStart(lines, index) && !startsAnimalStructuredContent(lines[index])) {
			append(' ')
			append(lines[index])
			index++
		}
	}
	return "$label: ${value.normalizeAnimalText()}" to index
}

private fun parseAnimalSectionLine(lines: List<String>, startIndex: Int): Pair<String, Int> {
	var index = startIndex
	val sectionTitle = lines[index]
	index++
	val sectionBody = buildString {
		while (index < lines.size && !isAnimalEntryStart(lines, index) && lines[index] !in animalSectionHeaders && !lines[index].startsWith(ANIMAL_CR_PREFIX)) {
			if (isNotEmpty()) append(' ')
			append(lines[index])
			index++
		}
	}
	return "$sectionTitle: ${sectionBody.normalizeAnimalText()}" to index
}

private fun isAnimalEntryStart(lines: List<String>, index: Int): Boolean {
	if (index + 2 >= lines.size) return false
	return lines[index].firstOrNull()?.isUpperCase() == true &&
		lines[index + 1].contains(',') &&
		lines[index + 2].startsWith(ANIMAL_AC_PREFIX)
}

private fun isAnimalBodyControlLine(lines: List<String>, index: Int): Boolean {
	if (index >= lines.size) return true
	val line = lines[index]
	return line.startsWith(ANIMAL_CR_PREFIX) || startsAnimalStructuredContent(line) || isAnimalEntryStart(lines, index)
}

private fun startsAnimalStructuredContent(line: String): Boolean {
	return animalProfileLabelFor(line) != null || line in animalSectionHeaders
}

private fun animalProfileLabelFor(line: String): String? {
	return animalProfileLabels.firstOrNull { label -> line.startsWith("$label ") }
}

private fun parseAcAndInitiative(line: String): Pair<String, String> {
	val match = Regex("""^${Regex.escape(MONSTER_STAT_AC)}\s+(.+?)\s+${Regex.escape(MONSTER_STAT_INITIATIVE)}\s+(.+)$""").matchEntire(line)
		?: error("Unable to parse AC/Initiative line: $line")
	return match.groupValues[1].trim() to match.groupValues[2].trim()
}

private fun parseAnimalAbilityLine(fragments: List<String>): String? {
	if (fragments.isEmpty()) return null
	val normalized = fragments.joinToString(" ").normalizeAnimalText()
	val matches = animalAbilityRegex.findAll(normalized).toList()
	if (matches.isEmpty()) return null
	return matches.joinToString(", ") { match ->
		val label = match.groupValues[1].uppercase()
		val score = match.groupValues[2]
		val modifier = match.groupValues[3].normalizeAnimalText()
		val save = match.groupValues[4].normalizeAnimalText()
		"$label $score ($modifier save $save)"
	}
}

private fun String.normalizeAnimalText(): String {
	return replace('−', '-')
		.replace('–', '-')
		.replace(Regex("""\s+"""), " ")
		.trim()
...
private fun monster(
	...
	return buildMonsterEntry(
		...
		cr = cr,
		body = body,
		group = MONSTER_GROUP_ANIMALS
	)
}

private val ANIMAL_MONSTER_RAW = """
Animals
Allosaurus
