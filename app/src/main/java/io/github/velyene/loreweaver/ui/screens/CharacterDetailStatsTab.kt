/*
 * FILE: CharacterDetailStatsTab.kt
 *
 * TABLE OF CONTENTS:
 * 1. Stats Tab Layout
 * 2. Ability and Skill Sections
 * 3. Dice and Roll Helpers
 */
...
fun StatsTab(
	...
	Row(verticalAlignment = Alignment.CenterVertically) {
		...
		OutlinedTextField(
			...
			onValueChange = { input ->
				val valid = input.isEmpty() || input == "-" || input.all { c -> c.isDigit() || c == '-' }
...
private fun SkillChips(
	...
	val skills = listOf(
		"Acrobatics" to character.dexterity,
		"Animal Handling" to character.wisdom,
		"Arcana" to character.intelligence,
		"Athletics" to character.strength,
		"Deception" to character.charisma,
		"History" to character.intelligence,
		"Insight" to character.wisdom,
		"Intimidation" to character.charisma,
		"Investigation" to character.intelligence,
		"Medicine" to character.wisdom,
		"Nature" to character.intelligence,
		"Perception" to character.wisdom,
		"Performance" to character.charisma,
		"Persuasion" to character.charisma,
		"Religion" to character.intelligence,
		"Sleight of Hand" to character.dexterity,
		"Stealth" to character.dexterity,
		"Survival" to character.wisdom
	)
	FlowRow(
		modifier = Modifier.fillMaxWidth(),
		horizontalArrangement = Arrangement.spacedBy(8.dp)
		...
		skills.forEach { (skill, baseScore) ->
			FilterChip(
				...
				onClick = {
					val situational = situationalBonus.toIntOrNull() ?: 0
					val bonus = character.getSkillBonus(skill, baseScore)
					onRollResult(skill to (Random.nextInt(1, 21) + bonus + situational))
				},
				label = { Text(skill) }
			)
		}
