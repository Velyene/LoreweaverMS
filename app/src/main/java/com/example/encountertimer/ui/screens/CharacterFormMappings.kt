...
import com.example.encountertimer.domain.model.ClassInfo
import com.example.encountertimer.domain.model.calcMaxHp
import com.example.encountertimer.domain.model.calcMaxMana
import com.example.encountertimer.domain.model.classInfoFor
import com.example.encountertimer.domain.model.normalizeClassName
import com.example.encountertimer.domain.model.roll4d6DropLowest
...
private const val PRIMARY_STAT_STRENGTH = "STR"
private const val PRIMARY_STAT_DEXTERITY = "DEX"
private const val PRIMARY_STAT_CONSTITUTION = "CON"

private val QUICK_BUILD_DEFAULT_PRIMARY_STATS =
	listOf(PRIMARY_STAT_STRENGTH, PRIMARY_STAT_CONSTITUTION)
...
internal fun CharacterFormState.quickBuild(classInfo: ClassInfo?): CharacterFormState {
	...
	val stats =
		mutableMapOf("STR" to 10, "DEX" to 10, "CON" to 10, "INT" to 10, "WIS" to 10, "CHA" to 10)
	...
	return copy(
		...
		con = stats["CON"].toString(),
		intell = stats["INT"].toString(),
		wis = stats["WIS"].toString(),
		cha = stats["CHA"].toString(),
...
private fun determineQuickBuildArmorClass(primaryStats: List<String>): String {
	return when {
		primaryStats.contains(PRIMARY_STAT_DEXTERITY) -> QUICK_BUILD_AC_DEX_PRIMARY
		primaryStats.contains(PRIMARY_STAT_STRENGTH) -> QUICK_BUILD_AC_STR_PRIMARY
