...
import com.example.encountertimer.domain.util.AbilityScoreSummary
import com.example.encountertimer.domain.util.BackgroundReference
...
import com.example.encountertimer.domain.util.LanguageReference
import com.example.encountertimer.domain.util.RaceReference
import com.example.encountertimer.domain.util.RacialTraitReference
import com.example.encountertimer.domain.util.ReferenceDetailResolver
import com.example.encountertimer.domain.util.ReferenceTable
import com.example.encountertimer.domain.util.SubraceReference
...
private data class CharacterCreationContentState(
	...
	val visibleTools: List<com.example.encountertimer.domain.util.ToolReferenceEntry>,
	val visibleWeapons: List<com.example.encountertimer.domain.util.WeaponReferenceEntry>,
	val visibleArmor: List<com.example.encountertimer.domain.util.ArmorReferenceEntry>,
	val visibleGear: List<com.example.encountertimer.domain.util.AdventuringGearEntry>,
	val visibleMagicItems: List<com.example.encountertimer.domain.util.MagicItemReferenceEntry>,
	val visibleAmmunition: List<com.example.encountertimer.domain.util.AmmunitionReferenceEntry>,
	val visibleFocuses: List<com.example.encountertimer.domain.util.FocusReferenceEntry>,
	val visibleMounts: List<com.example.encountertimer.domain.util.MountReferenceEntry>,
	val visibleTackAndDrawn: List<com.example.encountertimer.domain.util.TackDrawnReferenceEntry>,
	val visibleLargeVehicles: List<com.example.encountertimer.domain.util.LargeVehicleReferenceEntry>,
...
private fun rememberCharacterCreationContentState(
	...
	val equipmentChapterSections = remember(normalizedQuery) {
		com.example.encountertimer.domain.util.EquipmentReference.CHAPTER_SECTIONS.filter {
			it.matchesQuery(normalizedQuery)
		}
	}
	val weaponPropertySections = remember(normalizedQuery) {
		com.example.encountertimer.domain.util.EquipmentReference.WEAPON_PROPERTY_SECTIONS.filter {
			it.matchesQuery(normalizedQuery)
		}
	}
	val weaponMasterySections = remember(normalizedQuery) {
		com.example.encountertimer.domain.util.EquipmentReference.WEAPON_MASTERY_SECTIONS.filter {
			it.matchesQuery(normalizedQuery)
		}
	}
	val adventuringGearDetailSections = remember(normalizedQuery) {
		com.example.encountertimer.domain.util.EquipmentReference.ADVENTURING_GEAR_DETAIL_SECTIONS.filter {
			it.matchesQuery(normalizedQuery)
		}
	}
